...
Returns `{"run_id", "request_id", "status", "answer", "steps", "searches", "search_rewritten", "search_blocked", "trace": [...]}`. `status` is `finished`, `max_steps`, `blocked` (a planning turn failed the gate on a document, the question or the model's own text, or the local model server is unreachable) or `error`. With `?stream=1` the response is `text/event-stream`, one `event: <type>` per event:

| Event | Fields |
|---|---|
| `run_started` | `run_id`, `request_id`, `guard`, `max_steps`, `docs` (`id`, `title`), `question` |
| `hop` (upstream) | `hop`, `step`, `destination: "upstream"`, `kind`, `decision` (`allow`/`block`), `reasons`, `outbound` (messages new in this turn, as sent), `local` (the same messages as you have them), `model_used`, `ms`, and `detector: "retried"` or `"degraded"` when the local detector malfunctioned on this turn (below) |
...
| `final` | `status`, `answer` (rehydrated), `steps`, `searches`, `search_rewritten`, `search_allowed`, `search_blocked`, `results_withheld`, `detector_retried`, `detector_degraded` |
...
- **A degraded turn detects less.** When the local detector malfunctions twice on one turn, that turn goes out with deterministic detection only: a name or organization that appears for the first time in that turn, that no pattern, rule or GLiNER label covers, and that you did not declare, is sent. Values vaulted on earlier turns, declared terms, canaries and secret patterns are still enforced, and the audit marks the hop `detector_degraded`. An unreachable local model server still blocks the run.
...
| `AIRLOCK_ALLOW_UNGUARDED` | unset | Evaluation only: allows in-process `guard=False` agent runs (never over HTTP or the CLI, and ignored under `AIRLOCK_DEMO=1`) |
