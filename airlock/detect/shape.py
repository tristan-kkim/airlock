...
def trim(span: Span, text: str) -> Span | None:
    ...
    if span.type == "PERSON":
        if _HANGUL.search(new):
            ...
            new = re.sub(r"(?:['’]s)$", "", new)
    elif span.type == "SECRET" and (key := _CODE_KEY_HEAD.match(new)):
        # "DB_USER: b3JkZXJzX3J3" -> the value; the key is schema the answer refers to.
        if len(new) - key.end() >= MIN_SPAN_CHARS:
            new = new[key.end() :]
