...
_ORG_SUFFIX = (
    ...
    r"제약|증권|보험|캐피탈|물산|상사|로지스틱스|로지스|정밀|솔루션즈|솔루션|소프트|바이오|테크|"
    r"에너지|모빌리티|네트웍스|헬스케어|엔지니어링|중공업|화학|건설|식품|메디컬"
)
_ORG_TAIL = (
...
_ORG_GENERIC_STEMS: dict[str, frozenset[str]] = _stem_table(
    ...
    제약: 시간 예산 공간 기술 물리 자원 법적 법률 규제 설계 성능 메모리 하드웨어 운영 환경 조건
        ...
        구현 호환
    로지스: 글로벌 국제
    로지스틱스: 글로벌 국제 스마트 콜드체인 리버스
    정밀: 초 고 초고 극 반도체 기계 공작 측정 광학 의료 부품 금형
    솔루션: 보안 데이터 결제 물류 비즈니스 통합 토털 토탈 클라우드 인사 교육 마케팅 광고 에너지 IT
        빅데이터 모바일 헬스케어 스마트 AI 인공지능 고객 맞춤 맞춤형 원스톱 디지털
        소프트웨어 네트워크
        관리 업무 개발 문제 해결 최적 기술 자동화 문서 협업 기업 개인 금융 투자 보험 의료 건강
    솔루션즈: 글로벌 통합 비즈니스 IT
    소프트: 한글 오픈 프리 셰어 마이크로
    바이오: 레드 그린 화이트 합성 산업 의료 농업 해양 제약 식품 신약 첨단 국내 해외 글로벌
    테크: 핀 에듀 헬스 푸드 프롭 리걸 애드 펫 딥 하이 로우 클린 기후 그린 뷰티 스포츠 에이지 인슈어
        레그 섭 퀀텀 스페이스 바이오 메드 농업 스마트 첨단 신기술 고급 하이엔드
    에너지: 신재생 재생 대체 친환경 태양광 풍력 수소 원자력 청정 화석 전기 열 운동 위치 녹색 에코
        그린 스마트 분산 해양 바이오 지열 핵 소비 절약 고갈 긍정 부정 좋은 나쁜 넘치는 새로운 전체
    모빌리티: 도심 항공 개인형 마이크로 스마트 미래 공유 자율주행 전기 친환경 퍼스널 에어 UAM 차세대
    네트웍스: 소셜 글로벌
    헬스케어: 디지털 스마트 모바일 원격 개인 맞춤 맞춤형 예방 홈 AI 글로벌 국내 해외 반려동물 펫
    엔지니어링: 소프트웨어 리버스 역 사회공학 소셜 프롬프트 데이터 시스템 기계 화학 전기 전자 토목
        산업 유전 유전자 생명 생체 금융 품질 신뢰성 안전 가치 역설계
    중공업: 국내 글로벌
    화학: 유기 무기 분석 생 물리 고분자 정밀 석유 기초 응용 녹색 의약 전기 계산 양자 식품 환경 대기
        해양 지구 핵 방사 원자력 순수 일반 고등 대학 공업 산업 정유 여성 남성 첫 오묘한 둘 사이
    건설: 도시 주택 공공 해외 국내 민간 토목 플랜트 신도시 재개발 재건축 대형 중소 인프라 원전 SOC
        신규 모듈러 친환경 스마트 대교 교량 도로 철도 항만 공항 댐 기반 사회 국가 국토 공동 협동
    식품: 건강 가공 냉동 즉석 유기농 친환경 발효 기능성 수입 대체 간편 편의 반조리 조리 신선 저염
        저당 무첨가 전통 할랄 비건 수산 축산 농산 인스턴트 가정 영유아 이유 반려동물 특수
        의료 고령친화
        기호 일반 부패 상한 위해 불량 유해 해외 국산 국내 수입산 좋은 나쁜 어떤 모든 각종 이런 그런
    메디컬: 에스테틱 뷰티 스마트 디지털 글로벌
    주식회사:
    """
)
...
_FAMOUS_ORGS = frozenset(
    _words(
        ...
        대한의사협회 대한변호사협회 한국경영자총협회 한글과컴퓨터 삼성바이오로직스 셀트리온
        SK바이오팜 SK바이오사이언스 LG에너지솔루션 SK이노베이션 한화솔루션 삼성SDS
        포스코케미칼 LG화학 롯데케미칼
        한화에너지 SK에너지 GS에너지 두산에너빌리티 현대건설 삼성물산 대우건설 GS건설 DL이앤씨
        현대중공업 삼성중공업 HD현대중공업 한화오션 현대로템 쏘카 카카오모빌리티 네이버클라우드
        안랩 한컴 티맥스소프트 더존비즈온 코웨이 현대글로비스 CJ로지스틱스 롯데글로벌로지스
        """
    )
)
